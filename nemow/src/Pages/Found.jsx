...
function Found() {
  ...
  const HandleFilterByType = (type) => {
    ...
    if (type === "All") {
      setfilterdata(foundationdata);
    } else {
      let filterdata = foundationdata.filter((ele) => ele.name.includes(type));
  ...
  const HandleFilterByBrand = (brand) => {
    if (brand === "All") setfilterdata(foundationdata);
    else {
      let filterdata = foundationdata.filter((ele) => ele.brand.includes(brand));
      setfilterdata(filterdata);
    }
  };
  ...
  const HandleFilterByPrice = (type) => {
    if (type === "price") {
      ...
      let filterdata = foundationdata.filter(
      ...
      let filterdata = foundationdata.filter((ele) => ele.mrpRange.min >= 2000);
...
function EyeCareSection({ foundationdata = [] }) {
  ...
  return (
    ...
    >
      <Box>
        ...
        <Link>Foundation Creams and Powders</Link>
      </Box>
        ...
        <Accordion allowToggle>
          <AccordionItem>
            ...
            <AccordionPanel className="Filter_Parents">
              ...
              <Link
                onClick={() => HandleFilterByType("Powder")}
                className="Filter_TYpe_links"
              >
                Powder
              </Link>
